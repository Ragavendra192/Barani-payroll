...
from models.naps_employee import get_all_naps_employees, get_naps_by_emp_no
from models.naps_payroll import get_naps_payroll_month, save_naps_payroll_batch
from services.naps_payroll_service import calculate_naps_payroll
...
naps_payroll_bp = Blueprint("naps_payroll", __name__)

@naps_payroll_bp.route("/payroll/naps", methods=["GET", "POST"])
def manage_payroll():
    now = dt.datetime.now()
    ...
    # Check if NAPS payroll already exists for this month/year
    existing_payroll = get_naps_payroll_month(selected_year, selected_month)
    if existing_payroll:
        already_saved = True
    ...
    if request.method == "POST" or action == "load":
        ...
        if not valid:
            ...
            return render_template(
                "payroll/naps_payroll.html",
                year=selected_year, month=selected_month,
                standard_days=standard_days, payroll_rows=[],
                summary=None, already_saved=False
            )

        employees = get_all_naps_employees(status_filter="Active")
        
        if not employees:
            flash("No active NAPS employees found in database. Please add NAPS employees first.", "info")
        ...
        if action == "load":
            for emp in employees:
                ...
                calc = calculate_naps_payroll(emp, w_days, ot_h, other_ded)
            ...
            if action == "save":
                try:
                    save_naps_payroll_batch(selected_year, selected_month, payroll_rows)
                    month_name = dt.date(selected_year, selected_month, 1).strftime("%B")
                    flash(f"{month_name} {selected_year} NAPS Payroll saved successfully.", "success")
                    already_saved = True
                except Exception as e:
                    flash(f"Error saving NAPS payroll: {e}", "danger")

    # Compute summary totals
    if payroll_rows:
        summary = {
            "total_employees": len(payroll_rows),
            "total_gross": round(sum(r["Gross_Amount"] for r in payroll_rows), 2),
            ...
            "total_net": round(sum(r["Net_Amount"] for r in payroll_rows), 2),
        }
